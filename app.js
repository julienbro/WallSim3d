...
function init() {
    ...
    toggleSidebarButton.addEventListener('click', () => {
        ...
        toggleSidebarButton.textContent = isCollapsed ? '◀' : '▶'; // Update button text
    });

    // Section toggle functionality
    document.querySelectorAll('.toggle-section').forEach(button => {
        button.addEventListener('click', () => {
            const sectionContent = button.closest('.sidebar-section').querySelector('.section-content');
            sectionContent.classList.toggle('collapsed');
            button.textContent = sectionContent.classList.contains('collapsed') ? '▶' : '▼';
        });
    });
...
function handleToolbarClick(event) {
    ...
    // Navigation buttons
    if (buttonId === 'btn-orbit-left') { 
        rotateView(THREE.MathUtils.degToRad(10)); // Rotate left by 10 degrees
        return; 
    }
    if (buttonId === 'btn-orbit-right') { 
        rotateView(-0.1); // Rotate right
        return; 
    }
    if (buttonId === 'btn-zoom-in') { 
        zoomView(0.8); // Zoom in
        return; 
    }
    if (buttonId === 'btn-zoom-out') { 
        zoomView(1.2); // Zoom out
        return; 
    }
...
function placeElement(elementType, position, rotation) {
    ...
    updateElementCounter(); // Update the element counter
...
function triggerMove() {
    ...
    ghostElement.position.copy(selectedElement.mesh.position); // Start ghost at the current position of the selected element
    ghostElement.rotation.copy(selectedElement.mesh.rotation); // Match the rotation of the selected element
    ghostElement.visible = true;
...
function triggerDuplicate() {
 ...
 function onPointerMove(event) {
     // Disable ghost movement via pointer move
     if (currentTool === 'move' && moveState === 'moving') {
         return; // Prevent ghost position updates during move
     }
 ...
 function onPointerUp(event) {
     // Disable placement or movement confirmation via touch
     if (currentTool === 'move' && moveState === 'moving') {
         return; // Prevent move confirmation via touch
     }
 ...
 function handleElementSelectChange(event) {
     ...
     // Automatically switch to "add" tool when an element type is selected
     if (currentTool !== 'add') {
         setActiveTool('add');
     }
 ...
 function rotateView(angle) {
     ...
     controls.target.set(0, 15, 0); // Ensure the target is set correctly
     controls.update(); // Update the controls before applying rotation
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleLeftSidebarButton = document.getElementById('toggle-left-sidebar');

    // Ensure the toggle button is always visible
    toggleLeftSidebarButton.style.display = 'block';

    toggleLeftSidebarButton.addEventListener('click', () => {
        const isCollapsed = leftSidebar.classList.toggle('collapsed');
        toggleLeftSidebarButton.textContent = isCollapsed ? '▶' : '◀'; // Update button text
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleSidebarButton = document.getElementById('toggle-sidebar');
    const showRightSidebarButton = document.createElement('button');

    // Create the button to show the sidebar
    showRightSidebarButton.id = 'show-right-sidebar-button';
    showRightSidebarButton.textContent = '▶';
    document.body.appendChild(showRightSidebarButton);

    toggleSidebarButton.addEventListener('click', () => {
        const isCollapsed = rightSidebar.classList.toggle('collapsed-right');
        toggleSidebarButton.textContent = isCollapsed ? '◀' : '▶'; // Update button text
        showRightSidebarButton.style.display = isCollapsed ? 'flex' : 'none'; // Show or hide the show button
    });

    showRightSidebarButton.addEventListener('click', () => {
        if (rightSidebar.classList.contains('collapsed-right')) {
            rightSidebar.classList.remove('collapsed-right');
            toggleSidebarButton.textContent = '◀'; // Update button text
            showRightSidebarButton.style.display = 'none'; // Hide the show button
        }
    });
});

// Create and append the element counter to the DOM
document.addEventListener('DOMContentLoaded', () => {
    const elementCounter = document.createElement('div');
    elementCounter.id = 'element-counter';
    document.body.appendChild(elementCounter);
    updateElementCounter(); // Initialize the counter
});

/** Updates the element counter display */
function updateElementCounter() {
    const elementCounter = document.getElementById('element-counter');
    if (!elementCounter) return;

    elementCounter.innerHTML = ''; // Clear existing content

    // Add title
    const title = document.createElement('h3');
    title.textContent = 'Métré des matériaux';
    title.style.marginBottom = '10px';
    elementCounter.appendChild(title);

    if (placedElements.length === 0) {
        elementCounter.textContent = 'Aucun élément placé.';
        return;
    }
    ...
    placedElements.forEach(element => {
        const elementType = elementTypes[element.typeId] || { name: element.customName || 'Inconnu', dim: element.customDimensions || [0, 0, 0] };
        const typeName = elementType.name;
        if (!groupedElements[typeName]) {
            groupedElements[typeName] = { count: 0, dimensions: elementType.dim };
        }
        groupedElements[typeName].count++;
    });

    // Create a table for the element counter
    const table = document.createElement('table');
    table.classList.add('element-counter-table');
    ...
    // Add rows for each grouped type
    Object.entries(groupedElements).forEach(([typeName, data]) => {
        const [width, height, depth] = data.dimensions;
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${typeName}</td>
            <td>${width} x ${height} x ${depth} cm</td>
            <td>${data.count}</td>
        `;
        table.appendChild(row);
    });

    // Add the table to the counter
    elementCounter.appendChild(table);

    // Add dropdown functionality if too many rows
    const maxVisibleRows = 10; // Maximum number of rows before enabling dropdown
    if (Object.keys(groupedElements).length > maxVisibleRows) {
        table.style.maxHeight = '200px'; // Limit table height
        table.style.overflowY = 'auto'; // Add vertical scrolling
    }
}

document.addEventListener('DOMContentLoaded', () => {
    // Create the file bar
    const fileBar = document.getElementById('file-bar') || document.createElement('div');
    fileBar.id = 'file-bar';
    ...
    fileButton.textContent = 'Fichier';

    const fileDropdown = document.createElement('div');
    fileDropdown.className = 'dropdown';

    const fileActions = [
        { label: 'Nouveau', action: () => alert('Nouveau fichier créé') },
        { label: 'Ouvrir', action: () => alert('Ouvrir un fichier') },
        { label: 'Enregistrer', action: () => alert('Fichier enregistré') },
        { label: 'Quitter', action: () => alert('Quitter l\'application') }
    ];

    fileActions.forEach(({ label, action }) => {
        const button = document.createElement('button');
        button.textContent = label;
        button.addEventListener('click', action);
        fileDropdown.appendChild(button);
    });

    fileMenu.appendChild(fileButton);
    fileMenu.appendChild(fileDropdown);
    fileBar.appendChild(fileMenu);
    ...
    editDropdown.className = 'dropdown';

    // Actions for Edition menu
    const editActions = [
        { label: 'Copier', action: () => alert('Copier l\'élément sélectionné') },
        { label: 'Coller', action: () => alert('Coller l\'élément copié') },
        { label: 'Rotation', action: () => alert('Rotation de l\'élément sélectionné') },
        { label: 'Déplacer', action: () => alert('Déplacer l\'élément sélectionné') },
        { label: 'Dupliquer', action: () => alert('Dupliquer l\'élément sélectionné') }
    ];

    editActions.forEach(({ label, action }) => {
        const button = document.createElement('button');
        button.textContent = label;
        button.addEventListener('click', action);
        editDropdown.appendChild(button);
    });

    editMenu.appendChild(editButton);
    editMenu.appendChild(editDropdown);
    fileBar.appendChild(editMenu);

    // Style menu
    const styleMenu = document.createElement('div');
    styleMenu.className = 'menu';

    const styleButton = document.createElement('button');
    styleButton.textContent = 'Style';

    const styleDropdown = document.createElement('div');
    styleDropdown.className = 'dropdown';

    // Actions for Style menu
    const styleActions = [
        { label: 'Éléments en couleur', action: setElementsColored },
        { label: 'Éléments en blanc', action: setElementsWhite },
        { label: 'Ombres', action: toggleShadows }
    ];

    styleActions.forEach(({ label, action }) => {
        const button = document.createElement('button');
        button.textContent = label;
        button.addEventListener('click', action);
        styleDropdown.appendChild(button);
    });

    styleMenu.appendChild(styleButton);
    styleMenu.appendChild(styleDropdown);
    fileBar.appendChild(styleMenu);
    ...
    // Actions for Aide menu
    const helpActions = [
        { label: 'Aide', action: () => alert('Voici l\'aide.') },
        { label: 'À propos', action: () => alert('À propos de cette application.') }
    ];

    helpActions.forEach(({ label, action }) => {
        const button = document.createElement('button');
        button.textContent = label;
        button.addEventListener('click', action);
        helpDropdown.appendChild(button);
    });

    helpMenu.appendChild(helpButton);
    helpMenu.appendChild(helpDropdown);
    fileBar.appendChild(helpMenu);

    document.body.insertBefore(fileBar, document.body.firstChild);
});

/** Sets all elements to their original colors */
function setElementsColored() {
    placedElements.forEach(element => {
        if (element.mesh && element.mesh.material) {
            const elementType = elementTypes[element.typeId];
            if (elementType) {
                element.mesh.material.color.setHex(elementType.color);
                element.mesh.material.needsUpdate = true;
            }
        }
    });
    console.log('Éléments remis en couleur.');
}

/** Sets all elements to white */
function setElementsWhite() {
    placedElements.forEach(element => {
        if (element.mesh && element.mesh.material) {
            element.mesh.material.color.setHex(0xFFFFFF); // Pure white color
            element.mesh.material.needsUpdate = true;
        }
    });
    console.log('Éléments mis en blanc.');
}

/** Toggles shadows for all elements */
function toggleShadows() {
    const shadowsEnabled = placedElements.some(element => element.mesh.castShadow);
    placedElements.forEach(element => {
        if (element.mesh) {
            element.mesh.castShadow = !shadowsEnabled;
            element.mesh.receiveShadow = !shadowsEnabled;
        }
    });
    console.log(shadowsEnabled ? 'Ombres désactivées.' : 'Ombres activées.');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adjust the top position of the sidebars to account for the top bar height
    const topBarHeight = document.getElementById('file-bar').offsetHeight || 0;

    if (leftSidebar) {
        leftSidebar.style.top = `${topBarHeight}px`;
    }

    if (rightSidebar) {
        rightSidebar.style.top = `${topBarHeight}px`;
    }
});
